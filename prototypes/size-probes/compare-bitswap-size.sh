#!/usr/bin/env bash
#
# Focused footprint report for the maintained direct Bitswap protocol library.

set -o pipefail

here="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
cd "$here/bitswap-probe" || exit 1

bin="target/release/bitswap-probe"
...
echo
echo "Bitswap size-probes"
echo "----------------------------------------------------------------------------------------------------------------"
printf '  %-20s %-20s %14s   %-12s  %-7s  %s\n' "probe" "feature" "size" "vs baseline" "deps" "point"
echo "----------------------------------------------------------------------------------------------------------------"
row baseline baseline "empty binary control"
row co-libp2p-bitswap bitswap "maintained direct Bitswap protocol component"

# UldrenLoom (iOS binding, Swift)

The **iOS / Apple** binding over the Uldren Loom C ABI, written in Swift - the path for iOS, iPadOS,
and macOS (any Swift project). It wraps `include/loom.h` (`libuldren_loom`) through Swift's C interop.

Licensed under **BUSL-1.1** - the binding embeds the engine (see the repo `LICENSE`).

## Build (macOS host / Swift toolchain)
...
- `LoomSql` - SQL session with typed `exec`, `execJson`, `execBytes`, `execAsync`, `query`, and
  `commit`.
- `LoomSqlBatch` - held-open batch with `exec`, `execBytes`, `commit`, `commitVcs`, and `abort`.

...
fn parse_principal_kind(value: &str) -> napi::Result<PrincipalKind> {
    match value {
...
fn parse_external_credential_kind(value: &str) -> napi::Result<ExternalCredentialKind> {
    ExternalCredentialKind::parse(value).map_err(reason)
}
...
fn parse_acl_effect(value: i32) -> napi::Result<AclEffect> {
    match value {
...
fn parse_acl_rights(mask: u32) -> napi::Result<Vec<AclRight>> {
...
fn parse_acl_subject(value: &str) -> napi::Result<AclSubject> {
    match value {
        "*" | "everyone" => Ok(AclSubject::Everyone),
        role if role.starts_with("role:") => Ok(AclSubject::Role(
            WorkspaceId::parse(&role[5..]).map_err(reason)?,
        )),
        other => Ok(AclSubject::Principal(
            WorkspaceId::parse(other).map_err(reason)?,
        )),
    }
}
...
fn open_control_read(
    ...
    auth_principal: Option<String>,
    auth_passphrase: Option<String>,
) -> napi::Result<Loom<FileStore>> {
    let loom = open_loom_read_unlocked(path, key_spec(passphrase).as_ref()).map_err(reason)?;
    let auth = local_auth(auth_principal, auth_passphrase)?;
    attach_local_auth(loom, &auth).map_err(reason)
}
...
fn open_control_write(
    ...
    let loom = open_loom_unlocked(path, key_spec(passphrase).as_ref()).map_err(reason)?;
    let auth = local_auth(auth_principal, auth_passphrase)?;
    attach_local_auth(loom, &auth).map_err(reason)
}
...
fn decode_hex(value: &str) -> napi::Result<Vec<u8>> {
...
fn hex_nibble(byte: u8) -> napi::Result<u8> {
    match byte {
        b'0'..=b'9' => Ok(byte - b'0'),
        b'a'..=b'f' => Ok(byte - b'a' + 10),
        b'A'..=b'F' => Ok(byte - b'A' + 10),
        _ => Err(napi::Error::from_reason(
            "hex input contains a non-hex digit".to_string(),
        )),
    }
}
...
) -> napi::Result<Option<WorkspaceId>> {
    match workspace {
...
fn optional_acl_domain_arg(domain: Option<String>) -> napi::Result<Option<loom_core::AclDomain>> {
    match domain {
        Some(value) if !value.is_empty() => loom_core::AclDomain::parse(&value)
            .map(Some)
            .map_err(reason),
        _ => Ok(None),
    }
...
fn parse_acl_scope_kind(value: &str) -> napi::Result<AclScopeKind> {
    match value {
...
fn parse_acl_scope(value: &str) -> napi::Result<AclScope> {
...
fn parse_acl_scopes(scopes: Option<Vec<String>>) -> napi::Result<Vec<AclScope>> {
    match scopes {
...
fn optional_acl_predicate(value: Option<String>) -> napi::Result<Option<AclPredicate>> {
    value
        .filter(|value| !value.is_empty())
        .map(|value| AclPredicate::cel(value).map_err(reason))
        .transpose()
}
...
pub fn create_loom(
    ...
    let store = match passphrase.filter(|p| !p.is_empty()) {
        None => FileStore::create_with_profile(&path, digest_algo).map_err(reason)?,
        Some(pass) => {
            let suite = match suite.as_deref() {
                Some(s) => Suite::parse(s).map_err(reason)?,
    ...
    acl.allow(AclSubject::Principal(root), None, None, [AclRight::Admin])
        .map_err(reason)?;
    store.save_acl_store(&acl).map_err(reason)?;
    Ok(())
}
...
#[napi]
pub fn identity_list_json(
    loom_path: String,
    passphrase: Option<String>,
    auth_principal: Option<String>,
    auth_passphrase: Option<String>,
) -> napi::Result<String> {
    let loom = open_control_read(
        &loom_path,
        passphrase.as_deref(),
        auth_principal,
        auth_passphrase,
    )?;
    loom.authorize_global_admin().map_err(reason)?;
    let identity = loom
        .identity_store()
        .ok_or_else(|| napi::Error::from_reason("identity store not initialized".to_string()))?;
...
pub fn identity_add_principal(
    ...
    let mut loom = open_control_write(
        &loom_path,
        passphrase.as_deref(),
        auth_principal,
        auth_passphrase,
    ...
    let snapshot = {
        let identity = loom.identity_store_mut().ok_or_else(|| {
            napi::Error::from_reason("identity store not initialized".to_string())
        })?;
        identity
...
pub fn identity_rename_principal_handle(
    ...
    loom.store().save_identity_store(&snapshot).map_err(reason)
}
...
pub fn identity_remove_principal(
    ...
    let snapshot = {
        ...
        identity.remove_principal(principal).map_err(reason)?;
        identity.clone()
    };
    loom.store().save_identity_store(&snapshot).map_err(reason)
}
...
pub fn identity_assign_role(
    ...
    loom.authorize_global_admin().map_err(reason)?;
    let principal = WorkspaceId::parse(&principal).map_err(reason)?;
    let role = WorkspaceId::parse(&role).map_err(reason)?;
    let snapshot = {
        let identity = loom.identity_store_mut().ok_or_else(|| {
            napi::Error::from_reason("identity store not initialized".to_string())
        })?;
        identity.assign_role(principal, role).map_err(reason)?;
        identity.clone()
    };
    loom.store().save_identity_store(&snapshot).map_err(reason)
}
...
pub fn identity_revoke_role(
    ...
    let (removed, snapshot) = {
        ...
        let removed = identity.revoke_role(principal, role).map_err(reason)?;
...
pub fn identity_revoke_external_credential(
    ...
    loom.authorize_global_admin().map_err(reason)?;
    let credential = WorkspaceId::parse(&credential).map_err(reason)?;
    let snapshot = {
        let identity = loom.identity_store_mut().ok_or_else(|| {
            napi::Error::from_reason("identity store not initialized".to_string())
        })?;
...
pub fn identity_revoke_public_key(
    ...
    loom.authorize_global_admin().map_err(reason)?;
    let key = WorkspaceId::parse(&key).map_err(reason)?;
    let snapshot = {
        let identity = loom.identity_store_mut().ok_or_else(|| {
            napi::Error::from_reason("identity store not initialized".to_string())
        })?;
        identity.revoke_public_key(key).map_err(reason)?;
        identity.clone()
    };
    loom.store().save_identity_store(&snapshot).map_err(reason)
}
...
pub fn acl_grant(
    ...
    let snapshot = {
        ...
        acl.grant(grant).map_err(reason)?;
        acl.clone()
    };
    loom.store().save_acl_store(&snapshot).map_err(reason)
}
...
pub fn protected_ref_list_json(
    ...
    Ok(protected_ref_policies_json(
        &loom.protected_ref_policies(ns).map_err(reason)?,
    ))
}
...
pub fn protected_ref_get_json(
    ...
    let ns = resolve_workspace_arg(&loom, &workspace)?;
    Ok(
        match loom.protected_ref_policy(ns, &ref_name).map_err(reason)? {
...
pub fn protected_ref_set(
    ...
    let actor = loom.effective_principal().map_err(reason)?;
    let ns = resolve_workspace_arg(&loom, &workspace)?;
...
pub fn key_add_wrap_keyed(
    ...
    let fs = open_store_for_key_update(&loom_path, &passphrase)?;
...
pub fn key_remove_wrap(
    ...
    let fs = open_store_for_key_update(&loom_path, &passphrase)?;
    fs.remove_wrap(index as usize, allow_no_recovery)
        .map_err(reason)
}
/// The library version.
#[napi]
...
/// `loom_capabilities`.
#[napi]
pub fn capabilities() -> Uint8Array {
    ...
    Uint8Array::from(set.to_cbor())
}
/// The runtime provider/profile report as canonical CBOR.
#[napi]
pub fn runtime_profile() -> Uint8Array {
    Uint8Array::from(loom_core::runtime_profile().to_cbor())
}
#[napi]
pub fn studio_surface_catalog_json(workspace: String, set: Option<String>) -> napi::Result<String> {
...
#[napi]
pub fn result_to_json(bytes: Uint8Array) -> napi::Result<String> {
    loom_result::result_to_json(&bytes).map_err(reason)
}
...
#[napi]
pub fn result_to_bridge_json(bytes: Uint8Array) -> napi::Result<String> {
    loom_result::to_bridge_json(&bytes).map_err(reason)
}
